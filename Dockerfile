...
RUN apt-get update -qq && \
    apt-get install --no-install-recommends -y curl git postgresql-client libvips libjemalloc2 sudo && \
...
RUN apt-get update -qq && \
    apt-get install --no-install-recommends -y build-essential git pkg-config libyaml-dev libpq-dev node-gyp python-is-python3 libvips && \
    rm -rf /var/lib/apt/lists /var/cache/apt/archives
...
# Entrypoint prepares the database and starts the ssh service.
ENTRYPOINT ["/rails/bin/docker-entrypoint-azure"]
