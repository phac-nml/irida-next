...
module WorkflowExecutions
  class CompletionServiceTest < ActiveStorageTestCase
    def setup
      ...
      @workflow_execution_not_completed = workflow_executions(:irida_next_example)
      @workflow_execution_with_samples = workflow_executions(:irida_next_example_completed_with_samples)

      # get a new secure token for each test
    ...
    test 'finalize non complete workflow_execution' do
      workflow_execution = @workflow_execution_not_completed
    ...
    test 'sample metadata on samples_workflow_executions' do
      ...
      make_and_upload_blob(filepath: output_json_file_path, blob_run_directory:)
      make_and_upload_blob(filepath: output_summary_file_path, blob_run_directory:)
      make_and_upload_blob(filepath: output_analysis1_file_path, blob_run_directory:)
      make_and_upload_blob(filepath: output_analysis2_file_path, blob_run_directory:)
      make_and_upload_blob(filepath: output_analysis3_file_path, blob_run_directory:)
