...
module Samples
  class MultipleDestroyServiceTest < ActiveSupport::TestCase
    def setup
      @user = users(:john_doe)
    ...
    test 'multiple destroy with one sample and correct permissions' do
      assert_difference -> { Sample.count } => -1 do
        Samples::MultipleDestroyService.new(@project, [@sample1.id], @user).execute
      end
    end

    test 'multiple destroy with multiple samples and correct permissions' do
      assert_difference -> { Sample.count } => -3 do
        Samples::MultipleDestroyService.new(@project, [@sample1.id, @sample2.id, @sample30.id], @user).execute
      end
    end
    ...
    test 'valid authorization to destroy sample' do
      assert_authorized_to(:destroy_sample?, @project, with: ProjectPolicy, context: { user: @user }) do
        Samples::MultipleDestroyService.new(@project, [@sample1.id, @sample2.id, @sample30.id], @user).execute
      end
    end
