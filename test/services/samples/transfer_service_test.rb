...
module Samples
  class TransferServiceTest < ActiveSupport::TestCase
    ...
    test 'transfer project samples with permission' do
      @sample_transfer = SampleTransfer.new(project_id: @new_project.id,
                                            sample_ids: [JSON.generate([
                                                                         @sample1.id, @sample2.id
                                                                       ])])

      assert_changes -> { @sample1.reload.project.id }, to: @new_project.id do
        Samples::TransferService.new(@current_project, @john_doe).execute(@sample_transfer)
      end
    end

    test 'transfer project samples without specifying details' do
      assert_not Samples::TransferService.new(@current_project, @john_doe).execute(nil)
    end

    test 'transfer project samples to existing project' do
      @sample_transfer = SampleTransfer.new(project_id: @current_project.id,
                                            sample_ids: [JSON.generate([
                                                                         @sample1.id, @sample2.id
                                                                       ])])

      assert_not Samples::TransferService.new(@current_project, @john_doe).execute(@sample_transfer)
    end
    ...
    test 'transfer project samples without permission' do
      ...
      exception = assert_raises(ActionPolicy::Unauthorized) do
        Samples::TransferService.new(@current_project, @jane_doe).execute(@sample_transfer)
      end
