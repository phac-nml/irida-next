  ...
  class DialogComponentPreview < ViewComponent::Preview
    ...
    def default
      render(Pathogen::DialogComponent.new) do |dialog|
        ...
        dialog.with_body do
          tag.div do
            tag.p('This is a basic dialog with default settings. It uses medium size and is dismissible via the close button, ESC key, or clicking the backdrop.', class: 'mb-4') +
              tag.p('The dialog component provides a clean, accessible way to display modal content.')
          end
        end
        dialog.with_footer do
          tag.button('Cancel', type: 'button', class: 'px-4 py-2 text-sm font-medium text-slate-700 bg-white border border-slate-300 rounded-md hover:bg-slate-50') +
            tag.button('Confirm', type: 'button', class: 'ml-3 px-4 py-2 text-sm font-medium text-white bg-primary-600 rounded-md hover:bg-primary-700')
        end
      end
    end
    ...
    def small_dialog
      render(Pathogen::DialogComponent.new(size: :small)) do |dialog|
        ...
        dialog.with_footer do
          tag.button('Close', type: 'button', class: 'px-4 py-2 text-sm font-medium text-white bg-primary-600 rounded-md hover:bg-primary-700')
        end
      end
    end
    ...
    def large_dialog
      render(Pathogen::DialogComponent.new(size: :large)) do |dialog|
        ...
        dialog.with_body do
          tag.div do
            tag.p('This is a large dialog, suitable for more complex content like forms with multiple fields or detailed information displays.', class: 'mb-4') +
              tag.p('The larger width provides more space for content while maintaining readability.')
          end
        end
        dialog.with_footer do
          tag.button('Cancel', type: 'button', class: 'px-4 py-2 text-sm font-medium text-slate-700 bg-white border border-slate-300 rounded-md hover:bg-slate-50') +
            tag.button('Save', type: 'button', class: 'ml-3 px-4 py-2 text-sm font-medium text-white bg-primary-600 rounded-md hover:bg-primary-700')
        end
      end
    end
    ...
    def xlarge_dialog
      render(Pathogen::DialogComponent.new(size: :xlarge)) do |dialog|
        ...
        dialog.with_footer do
          tag.button('Done', type: 'button', class: 'px-4 py-2 text-sm font-medium text-white bg-primary-600 rounded-md hover:bg-primary-700')
        end
      end
    end
    ...
    def scrollable_content
      render(Pathogen::DialogComponent.new(size: :medium)) do |dialog|
        ...
        dialog.with_body do
          tag.div do
            # Search input
            search_input = tag.input(type: 'search', placeholder: 'Search items...', class: 'w-full px-3 py-2 border border-slate-300 rounded-md mb-4')
              ...
              20.times.map do |i|
                tag.div(class: 'flex items-center p-3 border border-slate-200 rounded-md hover:bg-slate-50') do
                  tag.input(type: 'checkbox', id: "item-#{i}", class: 'mr-3') +
                    tag.label("Item #{i + 1} - This is a sample item with some description", for: "item-#{i}", class: 'text-sm')
                end
              end.join.html_safe
            end
        ...
        dialog.with_footer do
          tag.button('Cancel', type: 'button', class: 'px-4 py-2 text-sm font-medium text-slate-700 bg-white border border-slate-300 rounded-md hover:bg-slate-50') +
            tag.button('Select', type: 'button', class: 'ml-3 px-4 py-2 text-sm font-medium text-white bg-primary-600 rounded-md hover:bg-primary-700')
        end
      end
    end
    ...
    def non_dismissible
      render(Pathogen::DialogComponent.new(dismissible: false)) do |dialog|
        ...
        dialog.with_body do
          tag.div do
            tag.p('This action cannot be undone. Please confirm that you want to proceed.', class: 'mb-4 text-slate-700') +
              tag.p('Note: This dialog cannot be dismissed by clicking outside or pressing ESC. You must click a button to proceed.', class: 'text-sm text-slate-500')
          end
        end
        dialog.with_footer do
          tag.button('Cancel', type: 'button', class: 'px-4 py-2 text-sm font-medium text-slate-700 bg-white border border-slate-300 rounded-md hover:bg-slate-50') +
            tag.button('Delete', type: 'button', class: 'ml-3 px-4 py-2 text-sm font-medium text-white bg-red-600 rounded-md hover:bg-red-700')
        end
      end
    end
    ...
    def complex_form
      render(Pathogen::DialogComponent.new(size: :large)) do |dialog|
          ...
          tag.form(class: 'space-y-4') do
            ...
            name_field = tag.div(class: 'space-y-1') do
              tag.label('Name', for: 'name', class: 'block text-sm font-medium text-slate-700') +
                tag.input(type: 'text', id: 'name', value: 'John Doe', class: 'w-full px-3 py-2 border border-slate-300 rounded-md')
            end

            # Email input
            email_field = tag.div(class: 'space-y-1') do
              tag.label('Email', for: 'email', class: 'block text-sm font-medium text-slate-700') +
                tag.input(type: 'email', id: 'email', value: 'john@example.com', class: 'w-full px-3 py-2 border border-slate-300 rounded-md')
            end
            ...
            dept_field = tag.div(class: 'space-y-1') do
              tag.label('Department', for: 'dept', class: 'block text-sm font-medium text-slate-700') +
                tag.select(class: 'w-full px-3 py-2 border border-slate-300 rounded-md', id: 'dept') do
                  %w[Engineering Marketing Sales Support].map { |dept| tag.option(dept, value: dept.downcase) }.join.html_safe
                end
            end
        ...
        dialog.with_footer do
          tag.button('Cancel', type: 'button', class: 'px-4 py-2 text-sm font-medium text-slate-700 bg-white border border-slate-300 rounded-md hover:bg-slate-50') +
            tag.button('Save Changes', type: 'button', class: 'ml-3 px-4 py-2 text-sm font-medium text-white bg-primary-600 rounded-md hover:bg-primary-700')
        end
      end
    end
