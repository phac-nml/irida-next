  ...
  module Samples
    class CloneTest < ActionDispatch::IntegrationTest
      ...
      test 'clone samples' do
        ...
        assert_samples_page(@project, 3)
        assert_samples_page(@project2, 22)
      ...
      test 'should not clone some samples' do
        ...
        assert_samples_page(destination, 2, namespace:)
        assert_samples_page(@project, 3)
        post_list([@sample1.id, @sample2.id, @sample30.id])

        assert_clone_enqueued destination: destination
      ...
      def assert_samples_page(project, count, namespace: nil)
      ...
      def assert_clone_enqueued(sample_ids: @project.samples.ids, destination: @project2)
        assert_enqueued_jobs 1, only: ::Samples::CloneJob do
      ...
      def post_clone(namespace_id: @namespace.id, sample_ids: @project.samples.ids, destination: @project2)
        post samples_clone_path,
             params: {
               namespace_id:,
               clone: { new_project_id: destination.id, sample_ids: },
