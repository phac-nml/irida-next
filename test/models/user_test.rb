...
class UserTest < ActiveSupport::TestCase
  ...
  test 'only remaining admin account cannot be removed' do
    @user.admin = true
    @user.save

    @user.destroy

    assert @user.errors.full_messages.include?(I18n.t('activerecord.errors.models.user.only_admin_account'))
  end
end
