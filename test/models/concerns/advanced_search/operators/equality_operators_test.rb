  ...
  module Operators
    class EqualityOperatorsTest < ActiveSupport::TestCase
      ...
      test 'condition_equals uses exact match for regular fields' do
        result = @test_instance.send(:condition_equals, @scope, @state_node, 'completed', field_name: 'state')
        sql = result.to_sql
      ...
      test 'condition_equals uses pattern match for metadata fields' do
        result = @test_instance.send(:condition_equals, @scope, @node, 'test', field_name: 'metadata.some_field')
        sql = result.to_sql
        assert_includes sql, 'ILIKE'
      end

      test 'condition_equals uses pattern match for name field' do
        result = @test_instance.send(:condition_equals, @scope, @node, 'test', field_name: 'name')
        sql = result.to_sql
        assert_includes sql, 'ILIKE'
      end
      ...
      test 'condition_equals uses case-insensitive exact match for workflow_version enum field' do
        result = @test_instance.send(:condition_equals, @scope, @node, '1.0.0', field_name: 'metadata.workflow_version')
        sql = result.to_sql
        assert_includes sql, 'LOWER'
        assert_not_includes sql, 'ILIKE'
      end
      ...
      test 'condition_not_equals uses not_eq for regular fields' do
        result = @test_instance.send(:condition_not_equals, @scope, @state_node, 'completed', field_name: 'state')
        sql = result.to_sql
        assert_includes sql, '"workflow_executions"."state" != '
      ...
      test 'condition_not_equals uses NOT ILIKE with null check for metadata fields' do
        result = @test_instance.send(:condition_not_equals, @scope, @node, 'test', field_name: 'metadata.some_field')
        sql = result.to_sql
      ...
      test 'condition_not_equals uses NOT ILIKE with null check for name field' do
        result = @test_instance.send(:condition_not_equals, @scope, @node, 'test', field_name: 'name')
        sql = result.to_sql
        assert_includes sql, 'NOT ILIKE'
        assert_includes sql, 'IS NULL'
      end
      ...
      test 'condition_not_equals uses case-insensitive not_eq for enum metadata fields' do
        result = @test_instance.send(:condition_not_equals,
                                     @scope, @node, 'phac-nml/pipeline', field_name: 'metadata.pipeline_id')
        sql = result.to_sql
        assert_includes sql, 'LOWER'
        assert_includes sql, '!='
        assert_includes sql, 'IS NULL'
