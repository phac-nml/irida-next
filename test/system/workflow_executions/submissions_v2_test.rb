...
module WorkflowExecutions
  ...
  class SubmissionsV2Test < ApplicationSystemTestCase
    ...
    test 'pagination does not render if only one page of samples' do
      ...
      # data-nextflow--v2--samplesheet-target="paginationContainer"
      assert_selector 'div[data-nextflow--v2--samplesheet-target="paginationContainer"]'
      assert_no_selector 'button[data-action="click->nextflow--v2--samplesheet#previousPage"][disabled]',
                         text: I18n.t('components.nextflow.samplesheet_component.previous')
      assert_no_selector 'select[data-action="change->nextflow--v2--samplesheet#pageSelected"]', text: '1'

      assert_no_selector 'button[data-action="click->nextflow--v2--samplesheet#nextPage"]',
                         text: I18n.t('components.nextflow.samplesheet_component.next')
    ...
    test 'samplesheet filtering empty state' do
      ...
      # verify empty state
      assert_selector 'div[data-nextflow--v2--samplesheet-target="emptyState"]'
    ...
    test 'samplesheet filter search and clear buttons' do
      ...
      assert_selector 'button[data-nextflow--v2--samplesheet-target="filterSearchButton"]'
      assert_no_selector 'button[data-nextflow--v2--samplesheet-target="filterClearButton"]'
      # enter filter and click search button
      find('input#samplesheet-filter').fill_in with: 'INXT_SAM_AAAAAAAAAC'
      find('button[data-nextflow--v2--samplesheet-target="filterSearchButton"]').click
      ...
      assert_no_selector 'button[data-nextflow--v2--samplesheet-target="filterSearchButton"]'
      assert_selector 'button[data-nextflow--v2--samplesheet-target="filterClearButton"]'

      # click clear button to remove filter
      find('button[data-nextflow--v2--samplesheet-target="filterClearButton"]').click
