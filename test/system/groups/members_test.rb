  ...
  class MembersTest < ApplicationSystemTestCase
    ...
    test 'can update member\'s access level to another access level' do
      ...
      Timecop.travel(Time.zone.now + 5) do
        ...
        assert_no_selector "table tbody tr[id='member_#{group_member.id}'] td:nth-child(2)",
                           text: I18n.t('activerecord.models.member.access_level.guest')
        assert_selector "table tbody tr[id='member_#{group_member.id}'] td:nth-child(2)",
    ...
    test 'cannot update member\'s access level to a lower level than what they have assigned in parent group' do
      ...
      assert_no_text I18n.t(:'concerns.membership_actions.update.success', user_email: group_member.user.email)
    end

    test 'can see the list of namespace group links' do
      namespace_group_link = namespace_group_links(:namespace_group_link6)

      visit group_members_path(namespace_group_link.namespace)
