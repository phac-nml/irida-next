...
module Projects
  class GroupLinksTest < ApplicationSystemTestCase
    ...
    test 'cannot remove a project to group link that may have been unlinked in another tab' do
      ...
      within table_row do
        assert_button I18n.t(:'projects.group_links.index.unlink')
    ...
    test 'can update namespace group links group access level to another access level' do
      ...
      Timecop.travel(Time.zone.now + 5) do
        ...
        within "table tbody tr[id='namespace_group_link_#{namespace_group_link.id}']" do
          assert_selector 'td:nth-child(4)', text: I18n.t('activerecord.models.member.access_level.maintainer')
        ...
        assert_no_selector "table tbody tr[id='namespace_group_link_#{namespace_group_link.id}'] td:nth-child(4)",
                           text: I18n.t('activerecord.models.member.access_level.maintainer')
    ...
    test 'can search group links by group name' do
      ...
      visit namespace_project_members_path(@namespace.parent, @project)
      click_button I18n.t(:'projects.members.index.tabs.groups')

      assert_text 'Displaying 4 items'
      assert_selector 'table tbody tr', count: 4

      fill_in placeholder: I18n.t(:'projects.group_links.invited_groups.search.placeholder'),
              with: @group_link2.group.name
