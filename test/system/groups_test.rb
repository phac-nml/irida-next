...
class GroupsTest < ApplicationSystemTestCase
  ...
  test 'should have Group URL filled with parent group, when creating new sub-group' do
    ...
    within %(div[data-controller="slugify"][data-controller-connected="true"]) do
      assert_selector %(input[data-select2--v1-target="input"]) do |input|
  ...
  test 'can transfer a group' do
    ...
    within %(form[action="/group-1/transfer"]) do
      assert_button I18n.t('groups.edit.advanced.transfer.submit'), disabled: true
      find_field(I18n.t('groups.edit.advanced.transfer.new_parent_id')).click
      find("li[data-label='#{group3.name}']").click
      assert_button I18n.t('groups.edit.advanced.transfer.submit'), disabled: false
  ...
  test 'empty state of transfer group' do
    ...
    within %(form[action="/group-1/transfer"]) do
      find_field(I18n.t('groups.edit.advanced.transfer.new_parent_id')).click
      fill_in I18n.t('groups.edit.advanced.transfer.new_parent_id'), with: 'invalid project name or puid'
