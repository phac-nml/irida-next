...
module W3cValidationHelpers
  ...
  def w3c_validate(name = 'caller', use_local: nil, validator_uri: DEF_W3C_VALIDATOR_PARAMS[:validator_uri],
    ...
    caller_info = format '%s:%d', bind.absolute_path.sub(%r{.*(/test/)}, '\1'), bind.lineno
    ...
    arerr = _may_ignore_autocomplete_errors_for_hidden(arerr,
                                                       "Ignores W3C validation errors for #{name} (#{caller_info}): ")
    arerr = _ignore_stray_start_tag_errors_for_turbo_stream(arerr,
                                                            "Ignores W3C validation errors for #{name} (#{caller_info}): ")
    arerr = _ignore_aria_errors_for_div_with_role_row(arerr,
                                                      "Ignores W3C validation errors for #{name} (#{caller_info}): ")
    arerr = _ignore_importmap_integrity_error(arerr, "Ignores W3C validation errors for #{name} (#{caller_info}): ")
    arerr = _ignore_aria_label_on_div_without_role(arerr,
                                                   "Ignores W3C validation errors for #{name} (#{caller_info}): ")
    arerr = _ignore_anchor_positioning_errors(arerr, "Ignores W3C validation errors for #{name} (#{caller_info}): ")
    arerr = _ignore_role_on_table_child_elements(arerr, "Ignores W3C validation errors for #{name} (#{caller_info}): ")
    assert_empty arerr,
                 "Failed for #{name} (#{caller_info}): W3C-HTML-validation-Errors(Size=#{arerr.size}): (" + arerr.map(&:to_s).join(') (') + ')'
  end
  ...
  # Botch fix of W3C validation errors for role attributes on tr/th/td inside a role="grid" table
  #
  # The pathogen DataGrid component uses role="grid" on the table element and explicitly sets
  # role="row", role="columnheader", role="gridcell" on tr/th/td elements for ARIA grid semantics.
  # The W3C Nu HTML Checker flags these as invalid because the native HTML elements already have
  # implicit ARIA roles when inside a table. These role attributes are semantically correct for
  # an accessible data grid, but the validator does not yet support this pattern.
  #
  # == References
  #
  # * ARIA Spec: https://www.w3.org/TR/wai-aria-1.2/#grid
  # * W3C Validator: https://github.com/validator/validator
  #
  # @param errs [Array<W3CValidators::Message>] Output of +@validator.validate_text(response.body).errors+
  # @param prefix [String] Prefix of the warning message recorded with Logger.
  ...
  unless method_defined?(:is_env_set_positive?)
    # true if the environmental variable is set and non-false
    def is_env_set_positive?(key)
      ENV.keys.include?(key.to_s) && !(%w[0 false FALSE f F] << '').include?(ENV.fetch(key, nil))
    end
  end
end
