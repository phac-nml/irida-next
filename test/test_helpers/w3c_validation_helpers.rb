...
module W3cValidationHelpers
  ...
  def w3c_validate(name = 'caller', use_local: nil, validator_uri: DEF_W3C_VALIDATOR_PARAMS[:validator_uri],
    ...
    arerr = _ignore_aria_label_on_div_without_role(arerr,
                                                   "Ignores W3C validation errors for #{name} (#{caller_info}): ")
    arerr = _ignore_anchor_positioning_errors(arerr,
                                              "Ignores W3C validation errors for #{name} (#{caller_info}): ")
    arerr = _ignore_aria_grid_role_errors_on_table_elements(arerr,
                                                            "Ignores W3C validation errors for #{name} (#{caller_info}): ")
    arerr = _ignore_aria_colindex_errors_on_table_elements(arerr,
                                                           "Ignores W3C validation errors for #{name} (#{caller_info}): ")
    assert_empty arerr,
