...
1. Pour le code que ce changement impacte, vous croyez que les tests automatisés valident les fonctionnalités qui sont très importantes pour les utilisateurs.
1. Si les tests automatisés existants ne couvrent pas la fonctionnalité ci-dessus, vous avez ajouté les tests nécessaires ou ajouté un problème pour décrire l'écart de test d'automatisation et l'avez lié à cette PR.
...
* Le premier réviseur de votre code, c'est *vous*. Avant d'effectuer cette première poussée de votre nouvelle branche brillante, lisez l'intégralité du diff. Est-ce que cela a du sens ? Avez-vous inclus quelque chose de non lié à l'objectif global des changements ? Avez-vous oublié de supprimer du code de débogage ?
...
* Récupérez la branche et testez les changements localement. Vous pouvez décider de l'étendue des tests manuels que vous souhaitez effectuer. Vos tests pourraient entraîner des opportunités d'ajouter des tests automatisés.
* Si vous ne comprenez pas un morceau de code, *dites-le*. Il y a de bonnes chances que quelqu'un d'autre soit également confus.
