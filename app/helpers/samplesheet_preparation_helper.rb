...
module SamplesheetPreparationHelper
  ...
  def execute_copy_step(workflow_execution, step2d) # rubocop:disable Metrics/AbcSize,Metrics/CyclomaticComplexity
    ...
    workflow_execution.samples_workflow_executions.find_each.with_index do |sample_workflow_execution, index_x|
      ...
      attachment_list.sort_by(&:id)&.[](start_y..)&.each&.with_index(start_y) do |attachment, index_y| # rubocop:disable Style/SafeNavigationChainLength
        compose_and_attach(attachment, sample_workflow_execution, workflow_execution.blob_run_directory)
  ...
  def execute_processing_step(workflow_execution) # rubocop:disable Metrics/MethodLength
    samplesheet_rows = process_samples_workflow_executions(workflow_execution)
    ...
    workflow_execution.inputs.attach(
      io: samplesheet_file(
        workflow_execution.workflow.samplesheet_headers,
        samplesheet_rows
      ),
      key: samplesheet_key,
      filename: 'samplesheet.csv'
  ...
  def compose_and_attach(attachment, attachable, blob_run_directory)
    key = generate_attachment_key(attachment, blob_run_directory)
  ...
  def process_samples_workflow_executions(workflow_execution)
    ...
    workflow_execution.samples_workflow_executions.each do |sample_workflow_execution|
      ...
      attachments.each do |key, attachment|
        samplesheet_params[key] = generate_blob_key(attachment, workflow_execution.blob_run_directory)
      end
  ...
  def generate_blob_key(attachment, blob_run_directory)
    blob_key_to_service_path(generate_attachment_key(attachment, blob_run_directory))
  end

  def generate_attachment_key(attachment, blob_run_directory)
    generate_input_key(
  ...
  def samplesheet_file(samplesheet_headers, samplesheet_rows)
