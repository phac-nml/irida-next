...
class GroupPolicy < NamespacePolicy # rubocop:disable Metrics/ClassLength
  ...
  scope_for :relation do |relation|
    relation.with(
      ...
      linked_groups: relation.where(id: NamespaceGroupLink.not_expired.where(Arel.sql('namespace_group_links.group_id in (select * from user_groups)')).select(:namespace_id)).self_and_descendant_ids
    ).where(
      Arel.sql(
