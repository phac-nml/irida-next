  ...
  class WorkflowExecutionsController < Groups::ApplicationController
    ...
    def page_title
      ...
      when 'show'
        @title = case @tab
                 when 'params'
                   "#{t(:'workflow_executions.show.tabs.params')} · #{t(:'shared.workflow_executions.workflow_execution')} #{@workflow_execution.id} · #{@group.full_path}"
                 when 'samplesheet'
                   "#{t(:'workflow_executions.show.tabs.samplesheet')} · #{t(:'shared.workflow_executions.workflow_execution')} #{@workflow_execution.id} · #{@group.full_path}"
                 when 'files'
                   "#{t(:'workflow_executions.show.tabs.files')} · #{t(:'shared.workflow_executions.workflow_execution')} #{@workflow_execution.id} · #{@group.full_path}"
                 else
                   "#{t(:'workflow_executions.show.tabs.summary')} · #{t(:'shared.workflow_executions.workflow_execution')} #{@workflow_execution.id} · #{@group.full_path}"
                 end
      end
    end
  end
