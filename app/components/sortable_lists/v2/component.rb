...
module SortableLists
  module V2
    # Server-rendered dual-list selector with checkbox controls and minimal JS behavior.
    class Component < ::Component
      attr_reader :title, :description, :templates, :template_label,
                  :required, :aria_live_translations, :instructions_id, :template_selector_id,
                  :interactive

      renders_many :lists, lambda { |id:, title:, list_role: nil, counterpart_list_id: nil, interactive: nil,
                                    **system_arguments|
        interactive = @interactive if interactive.nil?
        SortableLists::V2::ListComponent.new(
          id:,
          title:,
          required: @required,
      ...
      def initialize(title: nil, description: nil, templates: [], template_label: nil, required: false,
        ...
        @instructions_id = "sortable-lists-v2-instructions-#{SecureRandom.hex(6)}"
