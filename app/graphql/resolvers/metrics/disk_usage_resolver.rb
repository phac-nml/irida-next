  ...
  module Metrics
    ...
    class DiskUsageResolver < BaseResolver
      ...
      def resolve # rubocop:disable Metrics/AbcSize,Metrics/MethodLength
        ...
        # Collect distinct blob ids for attachments matching our three scopes
        ...
        total_bytes = if blob_ids.empty?
                        0
                      else
                        ActiveStorage::Blob.where(id: blob_ids).sum(:byte_size)
                      end

        number_to_human_size(total_bytes, precision: 2, significant: false, strip_insignificant_zeros: false)
      end
    end
  end
end
