...
module Pathogen
  ...
  class Datepicker < Pathogen::Component # rubocop:disable Metrics/ClassLength
    ...
    def initialize(id:, input_name:, label: nil, input_aria_label: nil, min_date: 1.day.from_now, # rubocop:disable Metrics/MethodLength
                   selected_date: nil, autosubmit: false, calendar_arguments: {}, wrapper_arguments: {},
                   **system_arguments)
    ...
    def setup_calendar_attributes
      ...
      @calendar_arguments[:data]["#{calendar_controller_name}-#{input_controller_name}-outlet"] = "##{@container_id}"
      @calendar_arguments[:data]["#{calendar_controller_name}-months-value"] = @months
    end

    # Configures HTML attributes for the <template> datepicker calendar.
    def setup_wrapper_attributes
      @wrapper_arguments[:tag] = if Flipper.enabled?(:beta_dropdown)
                                   TAG_DEFAULT
                                 else
                                   :template
                                 end
