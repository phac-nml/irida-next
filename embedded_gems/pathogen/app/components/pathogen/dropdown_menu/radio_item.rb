...
module Pathogen
  class DropdownMenu
    # Radio-like menu item for single selection.
    class RadioItem < Pathogen::Component
      ...
      def call
        ...
        input_id = self.class.generate_id(base_name: 'dropdown-menu-radio')
        ...
        data[:name] = @name
        data[:value] = @value
        data[:action] = class_names(existing_action, 'click->pathogen--dropdown-menu#selectRadio')
        ...
        wrapper << content_tag(
          ...
          radio_ui = content_tag(:span, '', class: class_names(
            'h-4 w-4 rounded-full border',
            'border-slate-300 dark:border-slate-600',
            'bg-primary-700 border-primary-700' => @checked
          ))

          radio_ui + content_tag(:span, @label, class: 'flex-1')
        end
